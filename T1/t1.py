...
while escolha_config != 's' and escolha_config != 'n':
	escolha_config = input("Deseja usar o tabuleiro do enunciado?(S/N): ").strip().lower()
	
custom_entradas = (escolha_config != 's')
if custom_entradas:
	arestas_ladders = []
	arestas_snakes = []
	m = eval(input("Quantidade de casas do lado do tabuleiro(n x n), n = "))
	...
	while True:
		escolha_aresta = eval(input("ARESTA COBRA: "))		
		...
		if (a <= b) or (a not in list(range(1, m*m+1))) or (b not in list(range(1, m*m+1))):
	...
	while True:
		escolha_aresta = eval(input("ARESTA ESCADA: "))		
		if escolha_aresta == 0: break		
		if type(escolha_aresta) != type((1,2)):
			print ("Formato incorreto, por favor digite no formato: a, b")
			continue
		...
		a, b = escolha_aresta[0], escolha_aresta[1]
		if (a >= b) or (a not in list(range(1, m*m))) or (b not in list(range(1, m*m))):
			print(("Insira no formato: a, b\nOnde a e b pertence a [1, %d] e (a < b), escadas avançam no tabuleiro." % (m*m-1)))
		else:
			arestas_ladders.append((a-1, b-1))
			
	k = eval(input("Iteracoes k do power method, k = "))
	a = eval(input("Fator a entre 0 e 1 para matriz pagerank, a = "))
	k_pagerank = eval(input("Iteracoes k do power method na matriz pagerank, k = "))
